...
namespace Cacophony
{
    ...
    {
        [Header("Variables")] [SerializeField] Vector3 _waveDirection;
        [SerializeField] float _waveLength;
        Vector3 _lastRayEndLocation;

        [Space] [Header("Curve")] [SerializeField]
        int points;

        [SerializeField] float amp;

        [Space] [Header("Layers")] [SerializeField]
        LayerMask _stopWaveAtLayers;
        ...
        [Space] [Header("Harmony Type")] [SerializeField]
        HarmonizationType _harmonyTypeOfWave;
        ...
        private void VisualizeWave(Vector3 specifiedDirection)
        {
            ...
            RaycastHit[] visualHarmonyArray =
                Physics.RaycastAll(transform.position, specifiedDirection, _waveLength, _stopWaveAtLayers);
            ...
            if (visualHarmonyArray.Length != 0)
            {
                ...
                foreach (RaycastHit currentVisualRay in visualHarmonyArray)
                {
                    ...
                    if (_lastHitInteractionObject != null &&
                        currentVisualRay.collider.gameObject != _lastHitInteractionObject)
                    {
                        RemoveAllChildWaves();
            ...
            SetEndOfVisualWave(transform.position +
                               new Vector3(specifiedDirection.x * _waveLength, 0, specifiedDirection.z * _waveLength));
        ...
        private bool HitEnemy(RaycastHit currentVisualRay)
        {
            ...
            if (currentVisualRay.collider.GetComponent<IHarmonizable>() != null)
            {
                ...
                GameplayManagers.Instance.Harmonizer.AddHarmonized(
                    currentVisualRay.collider.GetComponent<IHarmonizable>(), _harmonyTypeOfWave);

                if (currentVisualRay.collider.GetComponent<IHarmonizable>().GetHarmonizationType() !=
                    _harmonyTypeOfWave)
                    return false;
            }
        ...
        private void ReflectOff(GameObject reflectObject)
        {
            ...
            foreach (Vector3 harmonyDirection in reflectObject.GetComponentInParent<ReflectHarmony>()
                         .GetReflectDirection())
            {
        ...
        private HarmonyWave CreateVisualChildWave(GameObject hitObject, Vector3 direction,
            HarmonizationType harmonyType)
        {
            ...
            HarmonyWave recentChildWave = Instantiate(SelfPrefab, hitObject.transform.position, Quaternion.identity)
                .GetComponentInParent<HarmonyWave>();
