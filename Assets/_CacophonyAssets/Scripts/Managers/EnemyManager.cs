...
namespace Cacophony
{
    ...
    {
        [Header("Player")] [SerializeField] PlayerManager player;

        [SerializeField] Transform playerTransform;

        [Header("Enemy Materials")] [SerializeField]
        List<EnemyMaterials> _enemyMaterials;

        [Header("Enemy Stats")] [SerializeField]
        float timeToMove = 0.2f;

        [SerializeField] EnemySortType enemySortType = EnemySortType.DISTANCE_DESCENDING;
        ...
        private void SortEnemies(EnemySortType sortType)
        {
            switch (sortType)
            {
                ...
                case EnemySortType.DISTANCE_ASCENDING:
                    enemies = enemies.OrderBy(p => Vector3.Distance(p.transform.position, playerTransform.position))
                        .ToList();
                    return;
                case EnemySortType.DISTANCE_DESCENDING:
                    enemies = enemies
                        .OrderByDescending(p => Vector3.Distance(p.transform.position, playerTransform.position))
                        .ToList();
                    return;
            }
        }
    ...
    public class EnemyMaterials
    {
        ...
        [SerializeField] Material dissonantMaterial;

        [SerializeField] Material harmonizedMaterial;
