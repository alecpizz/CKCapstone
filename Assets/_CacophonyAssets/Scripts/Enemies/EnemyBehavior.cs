...
namespace Cacophony
{
    ...
    {
        [Header("AI")] [SerializeField] float timeToMove = 1;

        [SerializeField] float timeToRotate = 0.1f;

        [Tooltip("The amount of added cost for a tile if it is already claimed")] [SerializeField]
        float claimedCost = 1;

        [SerializeField] float prioritizedBonus = 1;
        internal List<GridStats> path { get; private set; } = new();

        [Space] [Header("Variables")] [SerializeField]
        bool _permaHarmonizable;
        ...
        [Header("Visuals")] [SerializeField] List<Renderer> renderers;
        ...
        private List<GridStats> GeneratePath(GridStats startTile, GridStats targetTile)
        {
            ...
            while (toSearch.Any())
            {
                ...
                foreach (var neighbor in current.Neighbors.Where(t =>
                             t.GetGridAvailability() != GridAvailability.Occupied && !processed.Contains(t)))
                {
                    ...
                    if (neighbor == playerTile)
                        ...
                        costToNeighbor = current.G + current.GetDistance(neighbor) +
                                         (neighbor.GetGridAvailability() == GridAvailability.Claimed ? claimedCost : 0);
        ...
        private Vector3 PredictNextPlayerPosition()
        {
            ...
            Vector3 predictedPos =
                (2 * enemyManager.GetPlayerTransform().position) - enemyManager.GetLastPlayerPosition();
