...
public class GridBehavior : MonoBehaviour
{
    ...
    [SerializeField]
    Material _startingPositionMaterial;
    ...
    {
        transform.position = new Vector3(transform.position.x - (float)_columns / 2, transform.position.y, transform.position.z - (float)_rows / 2);
        if (_columns % 2 != 0)
    ...
    {
        return new Vector3(transform.position.x + (x * _scale), transform.position.y, transform.position.z + (y * _scale));
    }
    ...
    public GridStats FindGridInDirection(Vector2 direction, GridStats startGrid)
    {
        ...
        return _gridArray[startGrid.GetXPos() + (int)direction.x, startGrid.GetYPos() + (int)direction.y].GetComponentInChildren<GridStats>();
    }
