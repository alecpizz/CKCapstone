...
{
    [SerializeField]
    float rotationTimeModifier = 0.5f;
    ...
    public bool MoveCharacter(GameObject objectToMove, Vector2 directionToMove, float moveTime)
    {
        ...
        if(CheckNewLocationValidity(objectToMove.transform.position, directionToMove, out standingSpace, out toGrid))
        {
    ...
    public bool MoveCharacter(GameObject objectToMove, GridStats toGrid, float moveTime)
    {
        ...
        GridStats standingSpace = GameplayManagers.Instance.Grid.DetermineGridSpaceFromPosition(objectToMove.transform.position);
    ...
    public bool CheckNewLocationValidity(Vector3 startpos, Vector2 directionToMove, out GridStats standingSpace, out GridStats toGrid)
    {
    ...
    private IEnumerator MoveCharacterOverTime(GameObject objectToMove, GridStats gridToMoveTo, float moveTime)
    {
        ...
        Vector3 gridPosY = new Vector3(gridToMoveTo.gameObject.transform.position.x, objectToMove.transform.position.y, gridToMoveTo.gameObject.transform.position.z);
    ...
    public IEnumerator RotateCharacterOverTime(GameObject objectToMove, GridStats gridToMoveTo, float moveTime)
    {
        ...
        Vector3 targetRotationEuler = new (0, Quaternion.LookRotation(targetPosition - startPosition).eulerAngles.y, 0);
        ...
        {
            time += Time.deltaTime / moveTime;
            objectToMove.transform.rotation = Quaternion.Lerp(objectToMove.transform.rotation, targetRotation, time);
